...
##############################
# TravelTimes related operations
##############################
...
def list_all_path_segments():
    ...
    path_segments = dictfetchall(cur, sql)
    print(path_segments)
...
# Get path segments that traverse a particular number of stops
def list_path_segments_by_stops(stops_count):
    conn = database_connect()
    if not conn:
        return None
    cur = conn.cursor()
    ...
    sql = f"""SELECT * 
             ...
             WHERE stopsTraversed = %s;"""
    
    path_segments = dictfetchall(cur, sql, (stops_count,))
    print(path_segments)
...
def add_path_segment(startstationid, endstationid, expectedtraveltimeSeconds, stopsTraversed, triplegs, coordinatemaplen):
    conn = database_connect()
    if not conn:
        return None
    cur = conn.cursor()

    sql = f"""INSERT INTO opaltravel.TravelTimes 
             (startstationid, endstationid, expectedtraveltimeSeconds, stopsTraversed, triplegs, coordinatemaplen)
             VALUES (%s, %s, %s, %s, %s, %s);"""
    print_sql_string(sql, (startstationid, endstationid, expectedtraveltimeSeconds, stopsTraversed, triplegs, coordinatemaplen))
    try:
        sql = f"""INSERT INTO opaltravel.TravelTimes 
             (startstationid, endstationid, expectedtraveltimeSeconds, stopsTraversed, triplegs, coordinatemaplen)
             VALUES (%s, %s, %s, %s, %s, %s);"""
...
def get_path_segment_by_ids(startstationid, endstationid):
    ...
    sql = f"""SELECT * FROM opaltravel.TravelTimes
...
def update_path_segment(start_station, end_station, expectedtraveltimeSeconds=None, stopsTraversed=None, triplegs=None, coordinatemaplen=None):
    conn = database_connect()
    if not conn:
        return None
    cur = conn.cursor()
    ...
    params = {
        ...
        'stopsTraversed': stopsTraversed,
        'triplegs': triplegs,
        'coordinatemaplen': coordinatemaplen
    }
    ...
    values.extend([start_station, end_station])
...
# Remove a path segment from the database

def remove_path_segment(start_station, end_station):
    """
    Remove a travel path segment from the system
    """
    # Data validation checks are assumed to have been done in route processing
    conn = database_connect()
    if conn is None:
    ...
    cur = conn.cursor()
    try:
        # Try executing the SQL and delete from the database
        sql = """
        DELETE FROM opaltravel.TravelTimes 
        WHERE startstationid = %s AND endstationid = %s;
        """

        cur.execute(sql, (start_station, end_station))
        conn.commit()  # Commit the transaction
        ...
        # If there were any errors, print an error to debug and re-raise the exception
        print("Unexpected error deleting path segment from", start_station, "to", end_station, sys.exc_info()[0])
        cur.close()    # Close the cursor
        conn.close()   # Close the connection to the db
        raise


# Produce a report showing stations with the longest expected travel time
def report_longest_travel_time():
    conn = database_connect()
    if not conn:
        return None
    cur = conn.cursor()

    sql = """SELECT startstationid, MAX(expectedtraveltimeSeconds) as max_travel_time
             FROM opaltravel.TravelTimes
             GROUP BY startstationid;"""
    report = dictfetchall(cur, sql)
